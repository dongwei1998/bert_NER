...
class Predictor(object):
    ...
    def predict_(self,queries):
        ...
        predictions = self.transformer(data, True, encode_padding_mask)

        labels_lists = tf.squeeze(tf.cast(predictions, dtype=tf.float32),axis=0).numpy().tolist()
        label_list = [labels.index(max(labels)) for labels in labels_lists]
        labels = [self.id_2_label[label_list[i]] for i in range(1, length - 1)]
        return dict(zip(inputs,labels))
...
if __name__ == '__main__':
    ...
    def predict():
        args.logger.info("基于bert模型：ner 命名体识别.....")
        try:
            # 参数获取
            ...
            # 直接调用预测的API
            predictions = detector.predict_(queries)
            return jsonify({
                'code': 200,
                'msg': '成功',
                'data': predictions
